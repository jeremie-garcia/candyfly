...
class Handle(Element):
    ...
    def drawText(self, event, qp):
        qp.setPen(self.textColor())
        qp.setFont(QtGui.QFont('Arial', 10))
        qp.drawText(event.rect(), QtCore.Qt.AlignLeft, str(int(self.main.start())))
        qp.drawText(event.rect(), QtCore.Qt.AlignRight, str(int(self.main.end())))
...
class QRangeSlider(QtWidgets.QWidget, Ui_Form):
    ...
    def setStart_silent(self, value):
        v = self._valueToPos(value)
        self._splitter.splitterMoved.disconnect()
        self._splitter.moveSplitter(v, self._SPLIT_START)
        self._splitter.splitterMoved.connect(self._handleMoveSplitter)
...
if __name__ == '__main__':
    ...
    rs.handle.setStyleSheet('background: blue;')
    app.exec_()
